...
try {
    const review = await reviewModel.find().populate({path: "products", select: "name description"}) // look what marco write
    res.send(review)
} catch (error) {
    next(error)
}
...
reviewsRouter.put("/:reviewId", async (req, res, next)=>{
    try {
        const reviewId = req.params.reviewId
        const modRev = await reviewModel.findByIdAndUpdate(reviewId, req.body,{new: true})
        if (modRev) {
            res.send(modRev)
        } else {
            next(createHttpError(404, `review with id ${req.params.reviewId} not fund`))
        }
    } catch (error) {
        next(error)
    }
